    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 animate-in fade-in slide-in-from-top duration-500 ${hidden && !open ? '-translate-y-full' : ''} ${scrolled ? 'border-b border-black/5 bg-[#fdfbf7]/90 backdrop-blur-md' : ''}`}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3.5">
        ...
        <nav className="hidden items-center gap-6 font-adam text-[0.7rem] uppercase tracking-[0.18em] md:flex">
          {NAV.map((n) =>
            n.internal ? (
              <Link key={n.label} href={n.href} className="text-[#2a1713]/70 transition hover:text-[#c94a3c]">{n.label}</Link>
            ) : (
              <a key={n.label} href={n.href} className="text-[#2a1713]/70 transition hover:text-[#c94a3c]">{n.label}</a>
            )
          )}
        </nav>
        <div className="hidden items-center gap-4 text-[#2a1713]/70 md:flex">
          <a href="/admin" className="flex items-center gap-1.5 text-[0.7rem] uppercase tracking-[0.14em] transition hover:text-[#c94a3c]">
        ...
        {/* Móvil: botón */}
        <button onClick={() => setOpen(true)} aria-label="Menú" className="flex size-10 items-center justify-center rounded-full bg-black/5 text-[#2a1713] backdrop-blur md:hidden">
