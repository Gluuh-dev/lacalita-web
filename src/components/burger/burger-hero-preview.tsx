    ...
    <div className="relative flex h-full items-center justify-center">
      ...
      {c.image && isVideoUrl(c.image) ? (
        <div key={`i${animKey}`} className="flex h-full items-center justify-center" style={{transform: `translateY(${c.mediaY ?? 0}%)`, zIndex: 2}}>
          <video src={c.image} autoPlay muted playsInline className="lc-slide-top" style={{height: '82%', maxWidth: '116%', objectFit: 'contain', WebkitMaskImage: 'radial-gradient(ellipse 96% 97% at 50% 50%, #000 90%, transparent 100%)', maskImage: 'radial-gradient(ellipse 96% 97% at 50% 50%, #000 90%, transparent 100%)'}} />
        </div>
      ) : c.image ? (
        <div key={`i${animKey}`} className="flex h-full items-center justify-center" style={{transform: `translateY(${c.mediaY ?? 0}%)`, zIndex: 2}}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={c.image} alt="" className="lc-slide-top" style={{height: '82%', maxWidth: '116%', objectFit: 'contain', WebkitMaskImage: 'radial-gradient(ellipse 96% 97% at 50% 50%, #000 90%, transparent 100%)', maskImage: 'radial-gradient(ellipse 96% 97% at 50% 50%, #000 90%, transparent 100%)'}} />
        </div>
      ) : (
