...
const TABS = [
  {label: 'Inicio', href: '/hamburgueseria', icon: Home, internal: true},
  {label: 'Carta', href: '/carta/hamburgueseria', icon: UtensilsCrossed, internal: true},
  {label: 'Ofertas', href: '#ofertas', icon: Percent, internal: false},
  {label: 'Local', href: '#local', icon: MapPin, internal: false}
];

export default function BurgerTabBar() {
  const pathname = usePathname();
  const isActive = (href: string) => (href === '/hamburgueseria' ? pathname === '/hamburgueseria' : pathname.startsWith(href));
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch justify-around border-t border-black/5 bg-[#fdfbf7]/95 pb-[max(0.3rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-md md:hidden">
      {TABS.map((t) => {
        const active = t.internal && isActive(t.href);
