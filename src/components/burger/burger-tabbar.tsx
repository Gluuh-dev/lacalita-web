...
export default function BurgerTabBar() {
  ...
  // En el detalle de producto hay barra de acciones propia (favorito + añadir).
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch justify-around border-t border-black/5 bg-[#fdfbf7]/95 pb-[max(0.3rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-md md:hidden">
      {items.map((t) => {
        const Icon = t.icon;
        const isList = t.label === 'Mi lista';
        const cls = `relative flex flex-1 flex-col items-center gap-0.5 py-1 text-[0.6rem] font-medium tracking-wide transition-colors ${isList && pop ? 'text-[#c94a3c]' : t.active ? 'text-[#c94a3c]' : 'text-[#2a1713]/55'}`;
        const inner = (
          <>
            <span className="relative">
              {isList && pop && <span className="lc-drop absolute left-1/2 top-[-9px] size-2.5 rounded-full bg-[#c94a3c]" />}
              <Icon className={`size-[21px] ${isList && pop ? 'lc-pop' : ''}`} strokeWidth={t.active ? 2.4 : 1.8} />
            </span>
            {t.label}
            {t.badge > 0 && (
              <span className={`absolute right-[18%] top-0 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#c94a3c] px-1 text-[0.58rem] font-bold text-[#fdfbf7] ${isList && pop ? 'lc-flash' : ''}`}>{t.badge}</span>
            )}
          </>
        );
        return t.href ? (
